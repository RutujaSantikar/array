    ...
    for (let i = 0; i < this.data1.length; i++) {
      ...
      for (let j = 0; j < calibrationReport.length; j++) {
        ...
        for (let k = 0; k < instrument.length; k++) {
          ...
          for (let l = 0; l < instrumentReport.length; l++) {
            const magnification = instrumentReport[l].magnification;

            if (magnification !== undefined) {
              for (let m = 0; m < magnification.length; m++) {
                const magId = magnification[m].id;
                for (let n = 0; n < this.data2.length; n++) {
                  for (let p = 0; p < this.data2[i].magnification.length; p++) {
                    if (this.data2[i].magnification[p].id === magId) {
                      console.log(this.data2[i].magnification[p]);
                    }
                  }
                }
              }
